    ...
    <div class="content-box">
      ...
      <div class="cell-list" v-show="kxData.length > 0">
        <div class="cell-item" v-for="item in kxData">
          <router-link :to="{ name:'meGongxiuList', params:{ id:item.id } , query:{ title:item.title }}" class="cell-link">{{ item.title }}</router-link>
          <p><cite style="float:none">已完成：<span>{{ item.completedCount }}</span> 部</cite></p>
        </div>
      </div>
